...
[`@tus/server`][tus Node.js] does not directly support SvelteKit endpoints, which
use the Fetch API's `Request` and `Response` objects.

However, you can integrate `@tus/server` by setting up a [custom server](https://svelte.dev/docs/kit/adapter-node#Custom-server). SvelteKit supports multiple
[adapters](https://kit.svelte.dev/docs/adapters), so choose the one that fits
your needs.
...
Transloadit’s strength is versatility. By handling video, audio, images, documents,
...
Start by creating a [server route](https://svelte.dev/docs/kit/routing#server) to generate the signature and params:
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (!authKey || !authSecret || !templateId) {
    return new Response(JSON.stringify({ error: 'Missing Transloadit credentials' }), { status: 500 });
  }
...
want to send values from Svelte state along to your endpoint, for instance, to add
