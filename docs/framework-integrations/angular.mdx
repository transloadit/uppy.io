...
`@uppy/angular` supports Angular 17, 18, 19, 20, 21 and 22.
...
[Angular + Uppy + Transloadit guide](/docs/guides/uppy-transloadit#angular). It
...
`UppyAngularDragDropModule` and `UppyAngularProgressBarModule` were removed in
Uppy 5.0. The [`@uppy/drag-drop`](/docs/drag-drop) plugin is still supported and
can be used directly with `uppy.use(DragDrop, { target })`.
