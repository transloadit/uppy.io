...
so there is a small risk involved in using the others.

`@uppy/aws-s3` is best suited for small files and/or lots of files.
If you are planning to upload mostly big files (100MB+), consider using [`@uppy/aws-s3-multipart`](/docs/upload-strategies/aws-s3-multipart).
