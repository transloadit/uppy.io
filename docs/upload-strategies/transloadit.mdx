...
It uses [Tus](/docs/upload-strategies/tus) under the hood so you don’t have to
...
You could for instance set it to`https://api2-us-east-1.transloadit.com` if you want to use a particular region.
...
*   `params` is used to authenticate with Transloadit and using your desired [template](https://transloadit.com/docs/topics/templates/).
    ...
    *   `steps` _(optional)_ can be used to [overrule Templates at runtime](https://transloadit.com/docs/topics/templates/#overruling-templates-at-runtime).
        ...
        For most use cases, we recommend to let your templates handle dynamic cases.
...
*   `signature` _(optional, but recommended)_ is a cryptographic signature to provide further trust in unstrusted environment.
...
The `${fields.caption}` variable will be available in the Assembly template.
...
*   When `false`, rhe Assemblies will complete (or error) in the background but Uppy won’t know or care about it.
*   When `true`, the Transloadit plugin waits for Assemblies to complete before the files are marked as completed.
    ...
    But, you can receive transcoding results on the client side, and have a fully client-side experience this way.
...
Wait for for Transloadit’s backend to catch early errors, not the entire template. (`Boolean`, default: `false`)
