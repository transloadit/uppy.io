...
- `false` - the default
- `"metaEditor"` - open the meta fields editor if [meta fields](/docs/dashboard/#metafields) are enabled.
- `"fileEditor"` - open the file editor if the file editor is enabled. Potentially there might be editors for many file formats, however currently we only support the [`@uppy/image-editor`](/docs/image-editor).
