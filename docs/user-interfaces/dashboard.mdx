...
Styles for Provider plugins, like Google Drive and Dropbox, are also bundled
...
Dashboard follows the core [`restrictions`](/docs/uppy/#restrictions) option
(file size, type filters, counts, etc.). Configure restrictions on your Uppy
instance and they will automatically apply to the Dashboard UI.
...
[`allowMultipleUploadBatches: false`](/docs/uppy/#allowmultipleuploadbatches)
option in Uppy Core to create a smooth experience when uploading a single (batch
of) file(s).
...
- [`@uppy/image-editor`](/docs/image-editor): allows users to crop, rotate, zoom
  and flip images that are added to Uppy.
