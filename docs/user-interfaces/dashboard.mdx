...
When only one file is selected, its preview and meta information will be centered
and enlarged (`boolean`, default: `true`).

Often times Uppy used for photo / profile image uploads, or maybe a single
document. Then it makes sense to occupy the whole space of the available Dashboard UI,
giving the stage to this one file. This feature is automatically disabled
when Dashboard is small in height, since there’s not enough room.
...
[the Dashboard example page](https://uppy.io/exampl/docs/dashboard/).
...
- [`@uppy/facebook`](/docs/dropbox) — import from
...
- [`@uppy/zoom`](/docs/url) — import files from [Zoom](https://zoom.us).
