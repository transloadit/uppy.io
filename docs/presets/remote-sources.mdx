...
`@uppy/remote-sources` is a preset plugin to add all the available remote
...
when you want to share the configuration between plugins. If you want your user
to upload files from any remote source, this plugin is recommended.
...
This plugin is published as the `@uppy/remote-sources` package on the npm
registry.
