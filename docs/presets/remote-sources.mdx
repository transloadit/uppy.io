...
plugins) to add all the available remote sources, such as Google Drive, Dropbox,
and others to Uppy Dashboard in one package.
...
`['Box', 'Dropbox', 'Facebook', 'GoogleDrive', 'OneDrive', 'Unsplash', 'Url', 'Zoom']`).
...
uppy.use(RemoteSources, {
	...
	sources: ['Dropbox', 'GoogleDrive', 'Unsplash', 'Url'],
});
```
