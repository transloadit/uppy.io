...
The `@uppy/compressor` plugin optimizes images (JPEG, PNG, and any other format supported by the client's browser) before upload, saving
up to 60% in size (roughly 18 MB for 10 images). It uses
[Compressor.js][] library under the
hood.
