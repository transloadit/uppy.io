...
- `IndexedDB` for small files, usually under 5MB
...
By default, Golden Retriever will only use the `IndexedDB` storage, which is good enough for files up to 5 megabytes (configurable). Service Worker is optional and requires additional setup.
