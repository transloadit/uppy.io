...
By default, Golden Retriever uses `IndexedDB` (or `LocalStorage` where IndexedDB
is unavailable), which is good enough for files up to 10 MiB. `Service Worker`
is optional and requires setup.
