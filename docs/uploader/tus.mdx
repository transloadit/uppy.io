...
When a request fails and the [retries](#retrydelays) are exhausted, the server’s
response is forwarded to the [`upload-error`](/docs/uppy/#upload-error) event
and set on `file.response`: the `status` the server returned, and a `body`
holding the underlying `xhr`.

```js
uppy.on('upload-error', (file, error, response) => {
	console.log(response?.status); // for instance 403
	console.log(response?.body.xhr.responseText); // what the server sent back
});
```

Before Uppy 6.0 the plugin aborted the request in its error handler and emitted
`upload-error` without a response at all, so this third argument was always
`undefined` and `file.response` was never set on a failed upload.

`response` is `undefined` when there is no server response to report: when the
request failed without one (such as an unreachable server; use
[`error.isNetworkError`](/docs/uppy/#upload-error) to detect those), and for
remote files uploaded through Companion, which report errors without a response
object.
