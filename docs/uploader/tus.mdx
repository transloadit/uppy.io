...
and set on `file.response`. It has the same shape as the response you get on
`upload-success`: the `status` the server returned, and a `body` with the
underlying `xhr`.
...
`response` is `undefined` when the request failed without a response, such as
when the server is unreachable. Use
[`error.isNetworkError`](/docs/uppy/#upload-error) to detect those.
