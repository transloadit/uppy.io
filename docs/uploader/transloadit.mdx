...
This one is never cleared, so you can keep showing the outcome of the previous
run after [`assemblyStatus`](#assemblystatus) goes back to `undefined`. It is
overwritten when the next Assembly reports a status.
