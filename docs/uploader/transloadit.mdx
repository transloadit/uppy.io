...
:::tip Canonical framework guide

Follow [Use Uppy with Transloadit](/docs/guides/uppy-transloadit) for complete,
signed examples in JavaScript, React, Next.js, Vue, and Angular. Uppy owns the
open-source upload experience; Transloadit owns the managed processing backend.
...
[`setMeta` method](/docs/uppy/#setmetadata). File metadata is set using the
[`setFileMeta`](/docs/uppy/#setfilemetafileid-data) method. The
...
[`getAssemblyOptions(file)`](/docs/transloadit/#getassemblyoptions) option.

</details>

### Plugin state

The plugin mirrors the Assembly Status into its own plugin state, so you can
render Assembly progress without wiring up event listeners. Read it with
[`getPluginState()`](/docs/uppy/#getpluginstate), or from Uppy’s state under
`plugins.Transloadit` (the key is the plugin [`id`](#id)).

```js
const { assemblyStatus, lastAssemblyStatus } = uppy
	.getPlugin('Transloadit')
	.getPluginState();
```

#### `assemblyStatus`

The [Assembly Status][assembly-status] of the Assembly that is currently running
(`object | undefined`, default: `undefined`).

It is updated on every status transition Transloadit reports (such as
`ASSEMBLY_UPLOADING` and then `ASSEMBLY_EXECUTING` in the `ok` property), and is
set back to `undefined` as soon as there is no live Assembly, which is the case
after the upload finished, was cancelled, or errored.

#### `lastAssemblyStatus`

The most recent [Assembly Status][assembly-status] that was seen
(`object | undefined`, default: `undefined`).

This one is never cleared, so it stays readable after
[`assemblyStatus`](#assemblystatus) goes back to `undefined`, and is overwritten
when the next Assembly reports a status. It is the last status the plugin
_observed_, not necessarily the final outcome: with the default
[`waitForEncoding: false`](#waitforencoding), the plugin stops following the
Assembly once the upload is done, so the stored status can still say
`ASSEMBLY_UPLOADING` or `ASSEMBLY_EXECUTING` while Transloadit finishes the job
server-side.
