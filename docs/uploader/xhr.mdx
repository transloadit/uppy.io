...
URL of the HTTP server (`string` or `function`, default: `null`).

The endpoint can also be derived from file data by providing a function.

```js
const endpoint = (fileOrBundle) => {
...
When [`formData`](#formdata) is set to true, this is used as the form field name
...
If the [`formData`](#formdata) option is set to false, `metaFields` is ignored.
...
- [`formData`](#formdata) must also be set to `true`.
...
[`uppy.setFileState()`](/docs/uppy/#setfilestatefileid-state) to set the
...
[`getResponseData()`](#getresponsedata) callback. This option sets the
[`XMLHttpRequest.responseType`][xhr.responsetype] property. Only `''`, `'text'`,
`'arraybuffer'`, `'blob'` and `'document'` are widely supported by browsers, so
it’s recommended to use one of those.
...
When using XHRUpload with [`formData: true`](#formdata), file metadata is sent
along with each upload request. You can set metadata for a file using
[`uppy.setFileMeta(fileID, data)`](/docs/uppy/#setfilemetafileid-data), or for
all files simultaneously using [`uppy.setMeta(data)`](/docs/uppy/#setmetadata).
...
[`uppy.setFileMeta(fileID, data)`](/docs/uppy/#setfilemetafileid-data) method to
do this:
...
Now, a form field named `size` will be sent along to the [`endpoint`](#endpoint)
...
sent to your upload endpoint, you can use the
[`allowedMetaFields`](#allowedmetafields) option to restrict the field names
that should be sent.
