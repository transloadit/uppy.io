...
Multipart uploads start to become valuable for larger files (100&nbsp;MB+) as it
...
- If you prefer to have less overhead (+20% upload speed)
  you can use temporary S3 credentials with
  [`getTemporary`](#gettemporarysecuritycredentialsoptions).
  This means users get a single token which allows them to do
  bucket operations for a longer time period, instead of short
  lived signed URL per resource. This is a security trade-off.
...
security credentials used for all uploads instead of signing every part.
This results in less request overhead which can lead to around 20% faster uploads.
This is a security tradeoff.

It’s strongly recomended to have some
