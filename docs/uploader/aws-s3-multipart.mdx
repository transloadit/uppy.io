...
A boolean, or an (async) function that returns a boolean which is called each
time a signed URL needs to be generated. It’s strongly recomended to have some
sort of caching process to avoid requesting more temporary token than necessary.
...
need to specify a function. See [`getTemporarySecurityCredentialsExpiry`][]
documentation for an example.

#### `getTemporarySecurityCredentialsExpiry(credentials, options)`

:::note

When using [Companion][companion docs] to generate temporary credentials, do not
define this option.

:::

An (async) function that returns when the given credentials will expire. This is
useful to let the client know when a signature has expired, and therefor cancel
the ongoing upload that has 0 chance to succeed. The reason this is not part of
`getTemporarySecurityCredentials()` is that we want the reponse from
`getTemporarySecurityCredentials` to be cacheable.

- `credentials`: `object` the credentials object returned by
  `getTemporarySecurityCredentials`
- `options`: `object`
  - `signal`: `AbortSignal`
- **Returns:** `number | Promise<number>` the validity time in second for the
  URL signature
...
uppy.use(AwsS3Multipart, {
	...
	async getTemporarySecurityCredentials(options) {
		const response = await fetch('/sts-token', options);
		if (!response.ok)
	...
	async getTemporarySecurityCredentialsExpiry(credentials, options) {
		const response = await fetch(
			'/sts-token-expiry?token=' + credentials.SessionToken.slice(0, 50),
			options,
		);
		if (!response.ok)
			throw new Error('Failed to fetch STS expiry', { cause: response });
		return response.json();
	},
});
```
...
[`gettemporarysecuritycredentialsexpiry`]:
	#getTemporarySecurityCredentialsExpiry-credentials-options
