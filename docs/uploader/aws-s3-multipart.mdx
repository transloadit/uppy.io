...
Uploads can be signed using either [Companion][companion docs], temporary credentials, or a custom
signing function.
...
Until the next major version, not setting this option uses the [legacy version of this plugin](./aws-s3).
This is a suboptimal experience for some of your user’s uploads.
It is best for speed and stability to upload large (100&nbsp;MB+) files with multipart
and small files with regular uploads.
...
It’s strongly recomended to have some sort of caching process to avoid
