...
As of Uppy 5.0, `@uppy/file-input` is deprecated. Prefer the headless hooks (e.g., `useDropzone`) from `@uppy/react`, `@uppy/vue`, or `@uppy/svelte`, or use [`@uppy/dashboard`](/docs/dashboard/). See the migration notes in the [migration guide](/docs/guides/migration-guides/).
...
import '@uppy/file-input/css/style.css';
...
Import general Core styles from `@uppy/core/css/style.css` first, then add the
File Input styles from `@uppy/file-input/css/style.css`. A minified version is
