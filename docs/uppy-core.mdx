...
`files-added` fire exactly once and a single failing file does not abort the
whole batch.
...
- Any remaining non-restriction errors are aggregated into an `AggregateError`
  (available on the thrown error’s `.errors` property) and thrown.

Prefer `addFiles` over calling `addFile` in a loop when adding multiple files:
batch listeners on `files-added` fire exactly once, and a single bad file does
not abort the batch.
