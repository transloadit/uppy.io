...
the system file dialog in UI plugins like `DragDrop` and `Dashboard`. When set
to `1`, they will only be able to select a single file. When `null` or another
number is provided, they will be able to select several files.
...
How long a notification will be visible (`number`, default: `5000`).
