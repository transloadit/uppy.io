...
`files-added` carrying a single-element array. When adding more than one file
at a time, prefer [`addFiles(files)`](#addfilesfiles) so that batch listeners
on `files-added` fire exactly once and a single failing file does not abort
the whole batch.
