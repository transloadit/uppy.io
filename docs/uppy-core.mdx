...
- `file.meta.relativePath`
  - For any local folder that was drag-dropped or opened in Uppy, the files
    ...
    selection (checkboxes). There are no leading or trailing slashes.
  ...
  - **Remote file example:** When checking the checkbox next to a remove
...
- `file.meta.absolutePath`
  - The `absolutePath` meta field will only be set for remote files. Regardless
    ...
    only exists for remote files, and always contains the full path to the file,
