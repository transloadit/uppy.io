...
checks failed. It throws on the first error, including restriction errors.

Every `addFile` call emits both `file-added` and `files-added`, with
`files-added` carrying a single-element array. When adding more than one file at
a time, prefer [`addFiles(files)`](#addfilesfiles) so that batch listeners on
`files-added` fire exactly once and a restriction failure on one file does not
...
uppy.addFile({
	...
	source: 'Local', // optional, determines the source of the file, for example, Dropbox.
	isRemote: false, // optional, set to true if actual file is not in the browser, but on some remote server, for example,
	// when using companion in combination with Dropbox.
});
```
...
Add multiple files to Uppy’s internal state in one call. Each entry in `files`
takes the same shape as the argument to [`addFile`](#addfilefile).

```js
uppy.addFiles([
	{ name: 'photo-1.jpg', type: 'image/jpeg', data: blob1 },
	{ name: 'photo-2.jpg', type: 'image/jpeg', data: blob2 },
]);
```

Events and errors behave differently from a loop of `addFile` calls, which is
the main reason to use this method for batch adds:

- Emits `file-added` once per accepted file.
- Emits `files-added` **once** for the whole batch, with the array of accepted
  files.
- Restriction failures (file too big, wrong type, too many files, etc.) emit
  `restriction-failed` for each failing file and surface an info message: **they
  do not throw**, and valid files in the same batch are still added.
- Any non-restriction error (for example, an exception thrown from
  `onBeforeFileAdded`) is aggregated into an `AggregateError` (available on the
  thrown error’s `.errors` property) and thrown **before state is updated**.
  When this happens, **no files from the batch are added** and `files-added` is
  not emitted.

Prefer `addFiles` over calling `addFile` in a loop when adding multiple files:
batch listeners on `files-added` fire exactly once, and restriction failures on
individual files do not abort the batch. Note that non-restriction errors still
abort the batch before any files are added, so if you need partial-success
semantics for arbitrary failures you’ll need to call `addFile` per file with
...
Calls `provider.logout()` on each remote provider plugin (Google Drive, Dropbox,
etc). Useful, for example, after your users log out of their account in your
app. This will clean things up with Uppy cloud providers as well, for extra
security.
...
Fired each time one or more files are added. A single
[`addFiles`](#addfilesfiles) call emits one `files-added` event for the whole
batch; an [`addFile`](#addfilefile) call emits one `files-added` event with a
single-element array. Looping `addFile` therefore produces N `files-added`
events; use `addFiles` instead if you want a single batch event.
...
[`onBeforeFileAdded`](#onbeforefileaddedfile-files). This will override the file
if it has not been uploaded. If you upload a duplicate file again it depends on
your upload plugin and backend how it is handled.
