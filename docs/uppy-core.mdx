...
If you want to see how it all comes together, checkout the examples on the [landing page](/).

### Working with Uppy files

Uppy keeps files in state with the [`File`][] browser API, but it is wrapped in an `Object`
...
With this option set to `false`, users can upload some files, and you can listen for the [`'complete'`](/docs/uppy/#complete)
...
Logger used for [`uppy.log`](#uppy-log) (`Object`, default: `justErrorsLogger`).

By providing your own `logger`, you can send the debug information to a server, choose to log errors only, etc.

:::note
Set `logger` to `Uppy.debugLogger` to get debug info output to the browser console:
...
| `minFileSize`        | `null`, `Number_` | minimum file size in bytes for each individual file                                                                              |
...
| `allowedFileTypes`   | `null`, `Array`   | wildcards `image/*`, or exact mime types `image/jpeg`, or file extensions `.jpg`: `['image/*', '.jpg', '.jpeg', '.png', '.gif']` |
| `requiredMetaFields` | `Array<string>`   | ...                                                                                                                              |
...
:::info
There are also two methods for updating `metadata`: [`setMeta`](#) and [`setFileMeta`](#).
:::

:::info
Metadata can also be added from a `<form>` element on your page,
through the [Form](/docs/form/) plugin or through the UI if you are using Dashboard with the [`metaFields`](/docs/dashboard/#metaFields) option.
:::
...
We recommend showing a message using [`uppy.info()`](#uppy-info) and logging to console
for debugging purposes via [`uppy.log()`](#uppy-log).
:::

<details>
<summary>Filter, modify, and abort example</summary>
...
<details>
<summary>Modify and abort example</summary>
...
:::info
If you want a different language it is better to use [locales](/docs/guides/using-locales).
:::
...
The store that is used to keep track of internal state (`Object`, default: [`DefaultStore`](/docs/stores/#DefaultStore)).
...
Get an array of all previously added [Uppy files](#working-with-uppy-files).
...
:::info
To change file data and such, use Uppy state updates, for example using [`setFileState`](#setFileState).
:::
...
as in [`uppy.upload()`](#uppy-upload)’s return value.
...
* `file` - The [Uppy file](#working-with-uppy-files) that failed to upload.
* `error` - The error object.
...
is to [`mount`](#mount) it to the DOM:
...
initialize [internationalisation](/docs/guides/using-locales).
...
Logger with additional debug and warning logs for during development.
...
`debugLogger` sends additional debugging and warning logs which could be helpful
during development:
...
[Dashboard]: /docs/user-interfaces/dashboard
