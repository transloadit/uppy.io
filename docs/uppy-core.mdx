...
You can also allow duplicate files with this.
...
Allow all files, also duplicate files.
This will override the file if it has not been uploaded.
If you upload a duplicate file again it depends on your upload plugin
and backend how it is handled.
...
| -------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `reason` | `string` | The reason for canceling. Plugins can use this to provide different cleanup behavior (Transloadit plugin cancels an Assembly if user clicked on the “cancel” button). Possible values are: `user` (default) - The user has pressed “cancel”; `unmount` - The Uppy instance has been closed programatically |
...
You can allow all files, even duplicate files, with [`onBeforeFileAdded`](#onbeforefileadded).
This will override the file if it has not been uploaded.
If you upload a duplicate file again it depends on your upload plugin
and backend how it is handled.
