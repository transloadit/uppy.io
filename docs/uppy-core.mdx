...
`@uppy/core` is an essential building block and is always required.
You can't install plugins without it. 
...
`@uppy/core` has four exports: `Uppy`, `UIPlugin`, `BasePlugin`, and `defaultLogger`.
