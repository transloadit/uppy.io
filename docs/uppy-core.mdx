...
uppy.addFile({
	...
	source: 'Local', // optional, determines the source of the file, for example, Google Drive.
	isRemote: false, // optional, set to true if actual file is not in the browser, but on some remote server, for example,
	// when using companion in combination with a remote provider.
});
```
...
- Restriction failures (file too big, wrong type, too many files, etc.) emit
  `restriction-failed` for each failing file and surface an info message —
  **they do not throw**, and valid files in the same batch are still added.
...
etc). Useful, for example, after your users log out of their account in your app
