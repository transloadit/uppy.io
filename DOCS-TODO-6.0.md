...
docs are single-version. Rewrite in place, old API lives in the migration guide,
deleted pages get client redirects.
...
- [ ] **`docs/uploader/aws-s3-multipart.mdx`**: full rewrite around the three
  ...
  - [ ] Keep only the survivors: `shouldUseMultipart`, `limit`, `getChunkSize`,
        `allowedMetaFields`, and note that `allowedMetaFields` currently only
...
- [ ] Decide: delete `docs/sources/companion-plugins/instagram.mdx` + add a
      `/docs/instagram` client redirect (Google Photos precedent in
...
- [ ] Angular page: confirm it mentions Angular 21 support (additive, peer range
      `^17` to `^21`; **not** a migration item).
