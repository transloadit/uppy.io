...
![The Uppy Dashboard after a page reload, showing a "Session restored — We restored all files. You can now resume the upload." notice above four recovered image thumbnails](/img/blog/1.29/golden-retriever-all-recovered.png)
...
![The Uppy Dashboard showing "Session restored — We could not fully recover 1 file. Please re-select it and resume the upload.". The unrecovered MP3 is greyed out with a ghost icon and a "Re-select" link, and the Upload button is disabled](/img/blog/1.29/golden-retriever-ghost.png)
...
![The Uppy Dashboard with local file selection turned off: no drop zone or browse link, just an "Import files from:" heading above a single Google Drive button](/img/blog/1.29/disableLocalFiles.png)
