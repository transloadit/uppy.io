...
Values remain in state until the next event (if that ever comes) or if the state is manually cleared. Depending on
...
sets the target origin for `window.postMessage()`, which is needed to communicate the
